...
Answer naturally using the data. Be concise but complete."""
        ...
        # Detect cascade chain from flattened tool results
        from api_registry import API_REGISTRY
        for called_tool in tools_called:
            tool_result = all_tool_results.get(called_tool, {})
            if not isinstance(tool_result, dict):
                continue
            current_tool = called_tool
            while current_tool:
                cfg = API_REGISTRY.get(current_tool, {}).get("cascade")
                if not cfg:
                    break
                trigger_val = tool_result.get(cfg["trigger_field"])
                if trigger_val and cfg["next_tool"] not in str(result["cascaded_tools"]):
                    result["cascaded_tools"].append(f"{current_tool} → {cfg['next_tool']}")
                    current_tool = cfg["next_tool"]
                else:
                    break
        if result["cascaded_tools"]:
            result["cascade_data"] = tool_result
